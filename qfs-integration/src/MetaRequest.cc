...
MetaAllocate::handle()
{
//    std::cout << "anonymous::MetaAllocate.handle 2148. layout: " << Show() << std::endl;
//    std::cout << "anonymous::MetaAllocate.handle 2149. next: " << MetaRequest::next << std::endl;
    assert(! MetaRequest::next);
    suspended = false;
//    std::cout << "anonymous::MetaAllocate.handle 2152. startedFlag: " << startedFlag << std::endl;
    if (startedFlag) {
        // TODO: we can maintian block->pathname here
        std::stringstream stream;
        stream << fid << "\."  << chunkId << "\." << chunkVersion;
        string blkname = stream.str();
        std::cout << "anonymous::MetaAllocate.handle 2158.map blk: " << blkname << " to obj: " << pathname << std::endl;
        gLayoutManager.mapBlk2Path(blkname, string(pathname.GetPtr()));
        return;
    }
    startedFlag = true;
//    std::cout << "anonymous::MetaAllocate.handle 2157. status: " << status << std::endl;
    if (status < 0) {
        return;
    }
    KFS_LOG_STREAM_DEBUG <<
...
//        std::cout << "anonymous::MetaAllocate.handle 2165.gWormMode not allowed!" << std::endl;
        statusMsg = "worm mode";
        status    = -EPERM;
        return;
    }
    if (! gLayoutManager.IsAllocationAllowed(*this)) {
//        std::cout << "anonymous::MetaAllocate.handle 2171" << std::endl;
        if (0 <= status) {
//            std::cout << "anonymous::MetaAllocate.handle 2173" << std::endl;
...
//        std::cout << "anonymous::MetaAllocate.handle 2184" << std::endl;
        SetEUserAndEGroup(*this);
    } else {
//        std::cout << "anonymous::MetaAllocate.handle 2187" << std::endl;
        euser = kKfsUserRoot; // Don't check permissions.
    }
    if (appendChunk && invalidateAllFlag) {
//        std::cout << "anonymous::MetaAllocate.handle 2191" << std::endl;
        statusMsg = "chunk invalidation is not supported with append";
        status    = -EINVAL;
        return;
    }
    if (appendChunk) {
//        std::cout << "anonymous::MetaAllocate.handle 2197" << std::endl;
    ...
    if (0 == numReplicas) {
//        std::cout << "anonymous::MetaAllocate.handle 2237" << std::endl;
        if (clientProtoVers <
                KFS_CLIENT_MIN_OBJECT_STORE_FILE_SUPPORT_PROTO_VERS) {
//            std::cout << "anonymous::MetaAllocate.handle 2240" << std::endl;
            status    = -EPERM;
            statusMsg = "client upgrade required to write object store file";
            return;
        }
        if (appendChunk) {
//            std::cout << "anonymous::MetaAllocate.handle 2246" << std::endl;
            status    = -EINVAL;
            statusMsg = "append is not supported with object store files";
            return;
        }
        if (invalidateAllFlag) {
//            std::cout << "anonymous::MetaAllocate.handle 2252" << std::endl;
            status    = -EINVAL;
...
//        std::cout << "anonymous::MetaAllocate.handle 2260" << std::endl;
        return; // Access denied or invalid request.
    }
    if (stripedFileFlag && appendChunk) {
//        std::cout << "anonymous::MetaAllocate.handle 2264" << std::endl;
        status    = -EINVAL;
        statusMsg = "append is not supported with striped files";
        return;
    }
    if (invalidateAllFlag) {
//        std::cout << "anonymous::MetaAllocate.handle 2270" << std::endl;
        if (! stripedFileFlag) {
//            std::cout << "anonymous::MetaAllocate.handle 2272" << std::endl;
            status    = -EINVAL;
        ...
        if (-EEXIST == status) {
//            std::cout << "anonymous::MetaAllocate.handle 2279" << std::endl;
            initialChunkVersion = chunkVersion;
            status = 0;
        }
        if (0 == status) {
//            std::cout << "anonymous::MetaAllocate.handle 2284" << std::endl;
...
//    std::cout << "anonymous::MetaAllocate.handle 2291" << std::endl;
    permissions = fa;
    minSTier    = fa->minSTier;
    maxSTier    = fa->maxSTier;
    if (status == -EEXIST) {
//        std::cout << "anonymous::MetaAllocate.handle 2296" << std::endl;
        initialChunkVersion = chunkVersion;
        // Attempt to obtain a new lease.
        status = 0;
        gLayoutManager.GetChunkWriteLease(*this);
        if (suspended) {
//            std::cout << "anonymous::MetaAllocate.handle 2302" << std::endl;
            panic("chunk allocation suspended after lease acquistion");
        }
        if (0 == status) {
//            std::cout << "anonymous::MetaAllocate.handle 2306" << std::endl;
            suspended = true;
            if (0 == numReplicas) {
//                std::cout << "anonymous::MetaAllocate.handle 2309" << std::endl;
                if (1 != servers.size() || ! servers.front()) {
//                    std::cout << "anonymous::MetaAllocate.handle 2311" << std::endl;
                    panic("chunk allocation suspended after lease acquistion");
                    status    = -EFAULT;
                    suspended = false;
                } else {
//                    std::cout << "anonymous::MetaAllocate.handle 2316" << std::endl;
                    servers.front()->AllocateChunk(*this, leaseId, minSTier);
                }
            } else {
//                std::cout << "anonymous::MetaAllocate.handle 2320" << std::endl;
                submit_request(new MetaLogChunkVersionChange(this));
            }
        }
        return;
    }
//    std::cout << "anonymous::MetaAllocate.handle 2326" << std::endl;
    suspended = true;
...
{
    std::cout << "anonymous::MetaChunkCorrupt.start 3505" << std::endl;
    if (server) {
        location = server->GetServerLocation();
        if (location.IsValid()) {
...
{
    std::cout << "anonymous::MetaChunkCorrupt.handle 3528: " << Show() << std::endl;
...
{
    std::cout << "anonymous::MetaChunkSize.handle 3668.handle" << std::endl;
