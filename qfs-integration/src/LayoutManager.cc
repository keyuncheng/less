...
{
//    std::cout << "anonymous::LayoutManager.UpdatePendingRecovery 122" << std::endl;
...
//        std::cout << "anonymous::LayoutManager.UpdatePendingRecovery 129" << std::endl;
        return;
    }
//    std::cout << "anonymous::LayoutManager.UpdatePendingRecovery 132" << std::endl;
    const MetaFattr* const fa = ent.GetFattr();
    for (CSMap::Entry* prev = csmap.Prev(ent);
            prev && prev->GetFattr() == fa; ) {
//        std::cout << "anonymous::LayoutManager.UpdatePendingRecovery 136" << std::endl;
...
//        std::cout << "anonymous::LayoutManager.UpdatePendingRecovery 142" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.UpdateReplicationState 151" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.InRecovery 201" << std::endl;
    return (
...
{
//    std::cout << "anonymous::LayoutManager.Invalidatechunk 226" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.ReplicaLost 684" << std::endl;
...
ChunkLeases::ReplicaLost(
    ...
    const ChunkServer*   chunkServer)
{
//    std::cout << "anonymous::LayoutManager.ReplicaLost 697" << std::endl;
    WriteLease& wl = we;
...
ChunkLeases::ServerDown(
    ...
    CSMap&                csmap)
{
//    std::cout << "anonymous::LayoutManager.ServerDown 723" << std::endl;
...
{
    std::cout << "anonymous::LayoutManager.FindServerByHost 1658.host: " << host << ", type: " << typeid(host).name() << std::endl;
...
{
    std::cout << "anonymous::LayoutManager.FindServerForReq 1672" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.UpdatePendingRecovery 1775" << std::endl;
...
LayoutManager::AddHosted(CSMap::Entry& entry, const ChunkServerPtr& c,
    size_t* srvCount)
{
//    std::cout << "anonymous::LayoutManager.AddHosted 1783" << std::endl;
    // Schedule replication even if the server went down, let recovery
    // logic decide what to do.
    UpdatePendingRecovery(entry);
//    std::cout << "anonymous::LayoutManager.AddHosted 1791" << std::endl;
    if (! c || c->IsDown()) {
//        std::cout << "anonymous::LayoutManager.AddHosted 1793" << std::endl;
        return false;
    }
//    std::cout << "anonymous::LayoutManager.AddHosted 1796" << std::endl;
    if (c->IsEvacuationScheduled(entry.GetChunkId())) {
//        std::cout << "anonymous::LayoutManager.AddHosted 1798" << std::endl;
        CheckReplication(entry);
    }
//    std::cout << "anonymous::LayoutManager.AddHosted 1801" << std::endl;
    const bool retFlag = mChunkToServerMap.AddServer(c, entry, srvCount);
//    std::cout << "anonymous::LayoutManager.AddHosted 1803" << std::endl;
    KFS_LOG_STREAM_DEBUG <<
...
{
//    std::cout << "anonymous::LayoutManager.AddHosted 1818" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.UpdateReplicationState 1818" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.SetReplicationState 1825" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.CheckReplicaiton 1841" << std::endl;
...
LayoutManager::SetParameters(const Properties& props, int clientPort)
{
    ...
    // integrate for oec
    useOEC = props.getValue("openec.useoec", "false");
    localIp = inet_addr(props.getValue("openec.localip", "0.0.0.0"));
    coorIp = inet_addr(props.getValue("openec.coorip", "0.0.0.0"));
...
{
//    std::cout << "anonymous::LayoutManager.UpdateDelayedRecovery 3610" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.AddServerWithStableReplica 3681" << std::endl;
    size_t     srvCount = 0;
...
{
//    std::cout << "anonymous::LayoutManager.RemoveServer 3717" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.Handle 3888" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.AddNotStableChunk 4765" << std::endl;
    CSMap::Entry* const cmi = mChunkToServerMap.Find(chunkId);
    if (! cmi) {
...
{
//    std::cout << "anonymous::LayoutManager.DumpChunkToServerMap 5112" << std::endl;
    //
...
{
//    std::cout << "anonymous::LayoutManager.CanBeRecovered 5189" << std::endl;
    cblk.clear();
...
void LayoutManager::mapBlk2Path(std::string blkname, std::string pathname) {
  blk2pathname.insert(make_pair(blkname, pathname));
}

std::string LayoutManager::getPath4Blk(std::string blkname) {
  assert (blk2pathname.find(blkname) != blk2pathname.end());
  return blk2pathname[blkname];
}

int
LayoutManager::AllocateChunkFromOEC(
    MetaAllocate& req, const vector<MetaChunkInfo*>& chunkBlock, bool allocateoec)
{
    string pathname = req.pathname.GetStr();
    std::cout << "anonymous::LayoutManager.AllocateChunkFromOEC 6600 for file " << pathname << ", numblks: " << chunkBlock.size() << std::endl;
    ...
    std::cout << "anonymous::LayoutManager.AllocateChunkFromOEC 6623.pathname: " << pathname << ", localIp: " << localIp << ", coorIp: " << coorIp << std::endl;
    ...
    std::cout << "anonymous::LayoutManager.AllocateChunkFromOEC 6660.allocate ip from oec is: " << targetloc << std::endl;
    delete coorCmd;

    // std::string targetloc("192.168.10.65");
    ChunkServerPtr targetserver;
    Servers::const_iterator const li = FindServerByHost(targetloc);
    assert(li != mChunkServers.end());
    targetserver = *li;

    req.servers.push_back(targetserver);
    tiers.push_back(placement.GetStorageTier());

    assert(!req.servers.empty() && req.status == 0 &&
        req.servers.size() <= (size_t)req.numReplicas &&
        req.servers.size() == tiers.size());

    if (! mChunkLeases.NewWriteLease(req)) {
        std::cout << "anonymous::LayoutManager::AllocateChunkFromOEC 6993 " << std::endl;
    ...
    if (mClientCSAuthRequiredFlag) {
         std::cout << "anonymous::LayoutManager::AllocateChunkFromOEC 6632 " << std::endl;
        ...
        std::cout << "anonymous::LayoutManager::AllocateChunkFromOEC 6637 " << std::endl;
        req.validForTime = 0;
    }
    req.allChunkServersShortRpcFlag = true;
    std::cout << "anonymous::LayoutManager::AllocateChunkFromOEC 6641.req.server.size: " <<req.servers.size()<< std::endl;
    ...
    for (size_t i = req.servers.size(); i-- > 0; ) {
        std::cout << "anonymous::LayoutManager::AllocateChunkFromOEC 6651 second: " << req.leaseId << ", tier: " << tiers[i] << std::endl;
        req.servers[i]->AllocateChunk(req, i == 0 ? req.leaseId : -1, tiers[i]);
    }
    std::cout << "anonymous::LyaoutManager::AllocateChunkFromOEC 6704.numblks:" << chunkBlock.size() << std::endl;
    return 0;
}
    ...
    MetaAllocate& req, const vector<MetaChunkInfo*>& chunkBlock)
{
    ...
    std::cout << "anonymous::LayoutManager.AllocateChunk 6595. chunkBlock.size: " << chunkBlock.size() << std::endl;
    ...
    kfsSTier_t minTier = req.minSTier;
    kfsSTier_t maxTier = req.maxSTier;
//    std::cout << "anonymous::LayoutManager.AllocateChunk 6625.minTier: " << (int)minTier << std::endl;
//    std::cout << "anonymous::LayoutManager.AllocateChunk 6625.maxTier: " << (int)maxTier << std::endl;
...
//        std::cout << "anonymous::LayoutManager::AllocateChunk 6640 " << std::endl;
        ...
        for (StripedFilesAllocationsInFlight::const_iterator it =
                ...
                ++it) {
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6651 " << std::endl;
            if (it->first.second == req.chunkBlockStart) {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6653 " << std::endl;
                const ChunkLeases::WriteLease* const lease =
                    mChunkLeases.GetChunkWriteLease(it->second);
                if (! lease || ! lease->chunkServer) {
//                    std::cout << "anonymous::LayoutManager::AllocateChunk 6657 " << std::endl;
                    continue;
                }
                if (lease->allocInFlight &&
                        lease->allocInFlight->status == 0) {
//                    std::cout << "anonymous::LayoutManager::AllocateChunk 6662 " << std::endl;
                    placement.ExcludeServerAndRack(
                        lease->allocInFlight->servers,
                        it->second);
                } else {
//                    std::cout << "anonymous::LayoutManager::AllocateChunk 6667 " << std::endl;
                    placement.ExcludeServerAndRack(
        ...
        for (vector<MetaChunkInfo*>::const_iterator it =
                ...
                it != chunkBlock.end();
                ++it) {
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6678 " << std::endl;
            Servers& srvs = serversTmp.Get();
            mChunkToServerMap.GetServers(GetCsEntry(**it), srvs);
            placement.ExcludeServerAndRack(srvs, (*it)->chunkId);
        }
    }
//    std::cout << "anonymous::LayoutManager::AllocateChunk 6684 " << std::endl;
    req.servers.reserve(req.numReplicas);
    StTmp<vector<kfsSTier_t> > tiersTmp(mPlacementTiersTmp);
    vector<kfsSTier_t>&        tiers = tiersTmp.Get();
...
//        std::cout << "anonymous::LayoutManager::AllocateChunk 6707 " << std::endl;
        replicaCnt++;
        localserver = *li;
 ...
 //       std::cout << "anonymous::LayoutManager::AllocateChunk 6717 " << std::endl;
        if (li != mChunkServers.end()) {
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6719 " << std::endl;
            rackIdToUse = (*li)->GetRack();
        }
        if (rackIdToUse < 0) {
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6723 " << std::endl;
            rackIdToUse = GetRackId(req);
        }
        if (rackIdToUse < 0 && li == mChunkServers.end()) {
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6727 " << std::endl;
            Servers::const_iterator const it = FindServerForReq(req);
            if (it != mChunkServers.end()) {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6730 " << std::endl;
                rackIdToUse = (*it)->GetRack();
            }
        }
    }
//    std::cout << "anonymous::LayoutManager::AllocateChunk 6735 " << std::endl;
    placement.FindCandidatesInRack(minTier, maxTier, rackIdToUse);
    size_t numServersPerRack(1);
    if (req.numReplicas > 1) {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 6739 " << std::endl;
    ...
    if ((req.appendChunk && ! mAppendPlacementIgnoreMasterSlaveFlag) ||
            localserver) {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 6758 " << std::endl;
    ...
    for (; ;) {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 6766 " << std::endl;
...
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6777 " << std::endl;
            const ChunkServerPtr cs = placement.GetNext(req.stripedFileFlag);
            if (! cs) {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6780 " << std::endl;
                break;
            }
            if (placement.IsUsingServerExcludes() &&
                    find(req.servers.begin(), req.servers.end(), cs) !=
                    req.servers.end()) {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6786 " << std::endl;
                continue;
            }
            numCandidates++;
            if (req.appendChunk && ! mAppendPlacementIgnoreMasterSlaveFlag) {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6791 " << std::endl;
...
//                    std::cout << "anonymous::LayoutManager::AllocateChunk 6797 " << std::endl;
                    if (req.servers.front()) {
//                        std::cout << "anonymous::LayoutManager::AllocateChunk 6799 " << std::endl;
                        mastersSkipped++;
                        continue;
                    }
...
//                    std::cout << "anonymous::LayoutManager::AllocateChunk 6806 " << std::endl;
                    if (req.servers.size() >=
                            (size_t)req.numReplicas) {
//                        std::cout << "anonymous::LayoutManager::AllocateChunk 6809 " << std::endl;
                        slavesSkipped++;
                        continue;
                    }
                    if (mAllocateDebugVerifyFlag &&
                            find(req.servers.begin(), req.servers.end(), cs) !=
                                req.servers.end()) {
//                        std::cout << "anonymous::LayoutManager::AllocateChunk 6816 " << std::endl;
                        panic("allocate: duplicate slave");
                        continue;
                    }
//                    std::cout << "anonymous::LayoutManager::AllocateChunk 6820 " << std::endl;
                    req.servers.push_back(cs);
                    tiers.push_back(placement.GetStorageTier());
                }
            } else {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6825 " << std::endl;
                if (mAllocateDebugVerifyFlag &&
                        find(req.servers.begin(), req.servers.end(), cs) !=
                            req.servers.end()) {
//                    std::cout << "anonymous::LayoutManager::AllocateChunk 6829 " << std::endl;
...
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6840 " << std::endl;
            break;
        }
        if (req.appendChunk && mInRackPlacementForAppendFlag && rackId >= 0 &&
                (req.numReplicas + 1) * placement.GetCandidateRackCount() <
                    mChunkServers.size()) {
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6846 " << std::endl;
...
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6856 " << std::endl;
                req.servers.push_back(localserver);
                tiers.push_back(minTier);
            }
...
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6863 " << std::endl;
...
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6878 " << std::endl;
            placement.ExcludeServer(
                req.servers.begin() + psz, req.servers.end());
        }
        if (! placement.NextRack()) {
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6883 " << std::endl;
            break;
        }
    }
    bool noMaster = false;
    if (req.servers.empty() || (noMaster = ! req.servers.front())) {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 6889 " << std::endl;
        ...
        for (Servers::const_iterator it = mChunkServers.begin();
                it != mChunkServers.end();
                ++it) {
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6899 " << std::endl;
            const ChunkServer& cs = **it;
            const int i = cs.CanBeChunkMaster() ? 0 : 1;
            if (! IsCandidateServer(cs)) {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6903 " << std::endl;
                dontLikeCount[i]++;
            }
            if (cs.GetAvailSpace() < mChunkAllocMinAvailSpace ||
                    ...
                    mMaxSpaceUtilizationThreshold) {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6910 " << std::endl;
                outOfSpaceCount[i]++;
            }
            if (! cs.IsResponsiveServer()) {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6914 " << std::endl;
                notResponsiveCount[i]++;
            }
            if (cs.IsHibernatingOrRetiring()) {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6918 " << std::endl;
                retiringCount[i]++;
            }
            if (cs.IsRestartScheduled()) {
//                std::cout << "anonymous::LayoutManager::AllocateChunk 6922 " << std::endl;
                restartingCount[i]++;
            }
//            std::cout << "anonymous::LayoutManager::AllocateChunk 6925 " << std::endl;
            KFS_LOG_STREAM_DEBUG <<
...
//        std::cout << "anonymous::LayoutManager::AllocateChunk 6950 " << std::endl;
...
//    std::cout << "anonymous::LayoutManager::AllocateChunk 6985 " << std::endl;
    assert(
    ...
    if (! mChunkLeases.NewWriteLease(req)) {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 6993 " << std::endl;
        panic("failed to get write lease for a new chunk");
        req.servers.clear();
        return -EFAULT;
    }
    ...
    if (req.stripedFileFlag) {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 7000 " << std::endl;
        if (! mStripedFilesAllocationsInFlight.insert(make_pair(make_pair(
                req.fid, req.chunkBlockStart), req.chunkId)).second) {
//            std::cout << "anonymous::LayoutManager::AllocateChunk 7003 " << std::endl;
            panic("duplicate in striped file allocation entry");
        }
    }
    if (mClientCSAuthRequiredFlag) {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 7008 " << std::endl;
        req.clientCSAllowClearTextFlag = mClientCSAllowClearTextFlag;
        req.issuedTime                 = TimeNow();
        req.validForTime               = mCSAccessValidForTimeSec;
    } else {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 7013 " << std::endl;
        req.validForTime = 0;
    }
    req.allChunkServersShortRpcFlag = true;
//    std::cout << "anonymous::LayoutManager::AllocateChunk 7017 " << std::endl;
    for (Servers::const_iterator it = req.servers.begin();
            it != req.servers.end();
            ++it) {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 7021 " << std::endl;
        if (! (*it)->IsShortRpcFormat()) {
//            std::cout << "anonymous::LayoutManager::AllocateChunk 7023 " << std::endl;
            req.allChunkServersShortRpcFlag = false;
            break;
        }
    }
    if (req.appendChunk) {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 7029 " << std::endl;
        mARAChunkCache.RequestNew(req);
    }
    for (size_t i = req.servers.size(); i-- > 0; ) {
//        std::cout << "anonymous::LayoutManager::AllocateChunk 7033 second: " << req.leaseId << ", tier: " << tiers[i]  << std::endl;
        req.servers[i]->AllocateChunk(req, i == 0 ? req.leaseId : -1, tiers[i]);
    }
    std::cout << "anonymous::LayoutManager.AllocateChunk 7153.numblks: " << chunkBlock.size() << std::endl;
    return 0;
}
...
{
    std::cout << "anonymous::LayoutManager.Handle MetaChunkCorrupt.8341" << std::endl;
    if (HandleReplay(req) || 0 != req.status) {
        std::cout << "anonymous::LayoutManager.Handle MetaChunkCorrupt.8343" << std::endl;
        return;
    }
    const char* p = req.chunkIdsStr.GetPtr();
    const char* e = p + req.chunkIdsStr.GetSize();
    std::cout << "anonymous::LayoutManager.Handle MetaChunkCorrupt.p: " << req.chunkIdsStr << std::endl;
    for (int i = -1; i < 0 || i < req.chunkCount; i++) {
        ...
        if (useOEC == "true") ChunkCorrupt(chunkId, req.server, kNotifyStaleFlag);
    }
}
...
{
    std::cout << "anonymous::LayoutManager.ChunkCorrupt 8367" << std::endl;
...
{
    std::cout << "anonymous::LayoutManager 9262.InitCheckAllChunks" << std::endl;
...
LayoutManager::Handle(MetaChunkSize& req)
{
    ...
    std::cout << "anonymous::LayoutManager.Handle MetaChunkSize 11053. finish writing blk: " << blkname << " of path: " << pathname << std::endl;
    if (std::find(corruptPaths.begin(), corruptPaths.end(), pathname) != corruptPaths.end()) {
        // report repaired to OpenEC
        CoorCommand* coorCmd = new CoorCommand();
        coorCmd->buildType11(11, localIp, pathname);
        coorCmd->sendTo(coorIp);
        delete coorCmd;
        // erase
        corruptPaths.erase(std::find(corruptPaths.begin(), corruptPaths.end(), pathname));
    }
...
{
    std::cout << "anonymous::LayoutManager.ReplicateChunk 11061" << std::endl;
    if (extraReplicas <= 0) {
        return 0;
    }
...
{
    std::cout << "anonymous::LayoutManager.ReplicateChunk 11151" << std::endl;
	...
	std::cout << "anonymous::LayoutManager.ReplicateChunk 11255" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11340" << std::endl;
    extraReplicas = 0;
    if (hibernatedReplicaCount) {
        *hibernatedReplicaCount = 0;
    }
    if (recoveryInfo) {
//        std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11347" << std::endl;
        recoveryInfo->Clear();
    }
    if (! mPrimaryFlag) {
//        std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11351" << std::endl;
        if (recoveryInfo) {
    ...
    const ChunkLeases::WriteLease* const wl =
        mChunkLeases.GetChunkWriteLease(chunkId);
    if (wl) {
//        std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11364" << std::endl;
        KFS_LOG_STREAM_DEBUG <<
    ...
    if (! IsChunkStable(chunkId)) {
//        std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11378" << std::endl;
        KFS_LOG_STREAM_DEBUG <<
            "re-replication delayed chunk:"
            " <" << c.GetFileId() << "," << chunkId << ">"
            " is not stable yet" <<
...
//        std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11390" << std::endl;
        KFS_LOG_STREAM_DEBUG <<
            "re-replication delayed chunk:"
            " <" << c.GetFileId() << "," << chunkId << ">"
            " file delete pending" <<
...
//        std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11399" << std::endl;
        KFS_LOG_STREAM_DEBUG <<
            "re-replication delayed chunk:"
            " <" << c.GetFileId() << "," << chunkId << ">"
...
//    std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11407" << std::endl;
    ...
    if (forceRecoveryFlag ||
            ...
            (mUseEvacuationRecoveryFlag &&
                ...
                fa->numReplicas == 1 &&
                fa->HasRecovery())) {
//        std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11429" << std::endl;
        if (! recoveryInfo || ! fa->HasRecovery()) {
//            std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11431" << std::endl;
            if (recoveryInfo) {
//                std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11433" << std::endl;
                KFS_LOG_STREAM_DEBUG <<
                ...
                // TODO report this chunk to OpenEC
                std::ostringstream o;
                o << chunk->id() << "\." << chunk->chunkId << "\." << chunk->chunkVersion;
                string lostblkname = o.str();
                string pathname = getPath4Blk(lostblkname);
                // add lost pathname to corruptpaths
                corruptPaths.push_back(pathname);
                // transfer blkname to filename
                std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11446.lostblkname: " << lostblkname  << ", pathname: " << pathname << std::endl;
                CoorCommand* coorCmd = new CoorCommand();
...
//        std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11444" << std::endl;
        StTmp<vector<MetaChunkInfo*> > cinfoTmp(mChunkInfosTmp);
        ...
        if (metatree.getalloc(fa->id(), offset,
                    mfa, mci, &cblk, &start) != 0 ||
                mfa != fa || mci != chunk) {
//            std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11455" << std::endl;
            panic("chunk mapping / getalloc mismatch");
            return false;
        }
//        std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11459" << std::endl;
        ...
        for (chunkOff_t pos = start;
                pos < end;
                pos += (chunkOff_t)CHUNKSIZE, stripeIdx++) {
//            std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11470" << std::endl;
            if (it == cblk.end()) {
//                std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11472" << std::endl;
                notStable = -1;
                break; // incomplete chunk block.
            }
//            std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11476" << std::endl;
            assert((*it)->offset % CHUNKSIZE == 0);
            if (pos < (*it)->offset) {
//                std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11479" << std::endl;
                if (fa->numStripes <= stripeIdx) {
//                    std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11481" << std::endl;
                    // No recovery: incomplete chunk block.
                    notStable = -1;
                    break;
                }
...
//            std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11490" << std::endl;
            if (holeFlag && stripeIdx < fa->numStripes) {
//                std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11492" << std::endl;
                // No prior stripes, incomplete chunk block.
                notStable = -1;
                break;
            }
            const chunkId_t curChunkId = (*it)->chunkId;
//            std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11498" << std::endl;
            if (mChunkLeases.GetChunkWriteLease(curChunkId) ||
                    ! IsChunkStable(curChunkId)) {
//                std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11501" << std::endl;
                notStable++;
                break;
            ...
            Servers&            srvs = serversTmp.Get();
            const CSMap::Entry& ce   = GetCsEntry(**it);
//            std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11509" << std::endl;
...
//        std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11521" << std::endl;
        if (notStable > 0 ||
                (notStable == 0 && good < (int)fa->numStripes)) {
//            std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11524" << std::endl;
            if (! servers.empty()) {
//                std::cout << "anonymous::LayoutManager.CanReplicateChunkNow 11526" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.CheckHibernating 11653" << std::endl;
    if (! mPrimaryFlag) {
        return;
    }
    ...
    for (HibernatedServerInfos::iterator iter = mHibernatingServers.begin();
            ...
            ++iter) {
//        std::cout << "anonymous::LayoutManager.CheckHibernating 11663" << std::endl;
        if (iter->removeOp) {
//            std::cout << "anonymous::LayoutManager.CheckHibernating 11665" << std::endl;
            continue;
        }
        Servers::const_iterator const it = FindServer(iter->location);
//        std::cout << "anonymous::LayoutManager.CheckHibernating 11669" << std::endl;
        if (it == mChunkServers.end() && now < iter->sleepEndTime) {
//            std::cout << "anonymous::LayoutManager.CheckHibernating 11671" << std::endl;
...
{
//    std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11734" << std::endl;
...
//    std::cout << "anonymous::LayoutManager.HandoutChunkReplicationWorker 11789.chunknum: " << mChunkToServerMap.Size() << std::endl;
    mChunkToServerMap.First(CSMap::Entry::kStateCheckReplication);
    for (; mPrimaryFlag; loopCount++) {
//        std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11792" << std::endl;
        if (--pass <= 0) {
//            std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11794" << std::endl;
            now  = microseconds();
            pass = kCheckTime;
            if (endTime <= now) {
//        	std::cout << "anonymous::layoutmanager.hnadoutchunkreplicationworker 11798" << std::endl;
                mReplicationCheckTimeouts++;
                if (nextRunLowPriorityFlag) {
//        	    std::cout << "anonymous::layoutmanager.hnadoutchunkreplicationworker 11801" << std::endl;
                    break;
                }
//        	std::cout << "anonymous::layoutmanager.hnadoutchunkreplicationworker 11804" << std::endl;
                timedOutFlag = true;
                mChunkReplicator.ScheduleNext();
                ...
                if (now < mLastReplicationCheckRunEndTime +
                        kMsgInterval) {
//        	    std::cout << "anonymous::layoutmanager.hnadoutchunkreplicationworker 11811" << std::endl;
                    break;
                }
                KFS_LOG_STREAM_INFO <<
...
//        std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11832" << std::endl;
...
//            std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11837" << std::endl;
            if (count <= 0) {
//        	std::cout << "anonymous::layoutmanager.hnadoutchunkreplicationworker 11839" << std::endl;
...
//        std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11860" << std::endl;
        CSMap::Entry* cur = mChunkToServerMap.Next(
            CSMap::Entry::kStateCheckReplication);
        if (! cur) {
            // See if all chunks check was requested.
//            std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11865. entry type: " << typeid(*cur).name() << std::endl;
            if (! (cur = mChunkToServerMap.Next(CSMap::Entry::kStateNone))) {
//        	std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11867" << std::endl;
                mCheckAllChunksInProgressFlag = false;
...
//                      std::cout << "anonymous::layoutmanager.HandoutChunkReplicationWorker 11874" << std::endl;
                    if (! (cur = mChunkToServerMap.Next(CSMap::Entry::kStateDelayedRecovery)))
//                      std::cout << "anonymous::layoutmanager.HandoutChunkReplicationWorker 11876" << std::endl;
...
//            std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11882" << std::endl;
            mChunkToServerMap.SetState(
                *cur, CSMap::Entry::kStateCheckReplication);
        }
        CSMap::Entry& entry = *cur;
//        std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11887" << std::endl;

        if (0 < GetInFlightChunkOpsCount(entry.GetChunkId(), kPendingOpTypes)) {
//            std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11890" << std::endl;
...
//        std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11898" << std::endl;
        int extraReplicas          = 0;
        int hibernatedReplicaCount = 0;
        recoveryInfo.Clear();
        ChunkPlacement& placement = placementTmp.Get();
        if (! CanReplicateChunkNow(
                ...
                &recoveryInfo)) {
            std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11909. cannot replicate chunk now!" << std::endl;
            continue;
        }
        if (extraReplicas > 0) {
//            std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11913" << std::endl;
            const int numStarted = ReplicateChunk(
                ...
                recoveryInfo);
            if (numStarted <= 0) {
//        	std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11920" << std::endl;
                SetReplicationState(entry, CSMap::Entry::kStateNoDestination);
            }
            count += numStarted;
            avail -= numStarted;
        } else {
//            std::cout << "anonymous::layoutmanager.Handoutchunkreplicationworker 11926" << std::endl;
            if (extraReplicas < 0) {
//        	std::cout << "anonymous::layoutmanager.hnadoutchunkreplicationworker 11928" << std::endl;
                DeleteAddlChunkReplicas(entry, -extraReplicas, placement);
            }
            if (hibernatedReplicaCount <= 0) {
//        	std::cout << "anonymous::layoutmanager.hnadoutchunkreplicationworker 11932" << std::endl;
...
{
//    std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 11964" << std::endl;
    if (! mPrimaryFlag) {
//        std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 11979" << std::endl;
        ScheduleCleanup(mMaxServerCleanupScan);
        return;
    }
//    std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 11983" << std::endl;
    if (! mPendingBeginMakeStable.IsEmpty() && ! InRecoveryPeriod()) {
//    	std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 11985" << std::endl;
        ProcessPendingBeginMakeStable();
    }
//    std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 11988" << std::endl;
...
//    std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 11993.recoveryFlag: " << recoveryFlag << std::endl;
//    std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 11993.fullCheckFlag: " << fullCheckFlag << std::endl;
    if (fullCheckFlag) {
//    	std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 11996" << std::endl;
    ...
    if (mLastReplicationCheckTime + mFullReplicationCheckInterval <= now) {
        ...
        KFS_LOG_EOM;
//    	std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 12004" << std::endl;
        InitCheckAllChunks();
...
//    std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 12012.runRebalanceFlag: " << runRebalanceFlag << std::endl;
    if (fullCheckFlag) {
//    	std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 12014" << std::endl;
        if (mMightHaveRetiringServersFlag) {
//    	    std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 12016" << std::endl;
...
//    std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 12024" << std::endl;
...
//    	std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 12028" << std::endl;
        mLastRebalanceRunTime = now;
        RebalanceServers();
    }
//    std::cout << "anonymous::LayoutManager.ChunkReplicationChecker 12032" << std::endl;
...
LayoutManager::Handle(MetaForceChunkReplication& op)
{
    ...
    if (dstIt != mChunkServers.end()) {
        ...
        extraReplicas = 1;
	std::cout << "anonymous::LayoutManager.MetaForceChunkReplication 13778" << std::endl;
        if (ReplicateChunk(
                *entry,
                extraReplicas,
                candidates,
