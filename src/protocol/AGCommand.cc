...
AGCommand::AGCommand(char *reqStr)
{
  ...
  // online object recovery
  case 13:
    resolveType13ForObjOnlineRecovery();
...
void AGCommand::resolveType3()
{
  ...
  for (int i = 0; i < targetnum; i++)
  {
    int target = readInt();
    vector<int> coef;
    for (int j = 0; j < _nprevs; j++)
      coef.push_back(readInt());
    int r = readInt();
                           ...
                           unordered_map<int, int> ref)
{
  ...
  for (int i = 0; i < _readCidList.size(); i++)
    writeInt(_readCidList[i]);
  writeInt(_nprevs);
  for (int i = 0; i < _nprevs; i++)
  {
    writeInt(_prevCids[i]);
...
void AGCommand::resolveType7()
{
  ...
  for (int i = 0; i < readnum; i++)
    _readCidList.push_back(readInt());
  _nprevs = readInt();
  for (int i = 0; i < _nprevs; i++)
  {
    _prevCids.push_back(readInt());
...
void AGCommand::buildType13ForObjOnlineRecovery(int type,
                                                unsigned int sendIp,
                                                string objName,
                                                int objSizeMB,
                                                int pktNum)
{
  ...
  _type = type;
  _sendIp = sendIp;
  _readObjName = objName;
  _filesizeMB = objSizeMB;
  _num = pktNum;
...
void AGCommand::resolveType13ForObjOnlineRecovery()
{
  _readObjName = readString();
  _filesizeMB = readInt();
  _num = readInt();
