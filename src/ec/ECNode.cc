...
{
  if (_refNumFor.find(id) == _refNumFor.end())
    _refNumFor.insert(make_pair(id, 1));
  else
    _refNumFor[id]++;
...
int ECNode::getCoefOfChildForParent(int child, int parent)
{
  ...
  {
    if (_childNodes[i]->_nodeId == child)
      return coefs[i];
  }
...
void ECNode::parseForClient(vector<ECTask *> &tasks)
{
  ...
  else
  {
    ...
    if (childNode->getCoefmap().size() > 1)
      computebool = false;
    // 2> this node is not linked to a bind node, there is calculation of res = coef * value
    // 3> if the childNode is a leaf node, the childNode->getCoefmap().size() == 0,
    // but we also need computation
    else if (childNode->getCoefmap().size() <= 1)
      computebool = true;
  }
  ...
  if (computebool)
  {
    ...
    vector<int> children;
    for (int i = 0; i < _childNodes.size(); i++)
      children.push_back(_childNodes[i]->getNodeId());
                                  ...
                                  unordered_map<int, unsigned int> cid2ip)
{
  ...
  if (_oecTasks.find(0) != _oecTasks.end())
    load = true;
  if (_oecTasks.find(1) != _oecTasks.end())
    fetch = true;
  if (_oecTasks.find(2) != _oecTasks.end())
    compute = true;
  if (_oecTasks.find(3) != _oecTasks.end())
    cache = true;
...
void ECNode::dumpRawTask()
{
  ...
  for (int i = 0; i < 4; i++)
  {
    if (_oecTasks.find(i) == _oecTasks.end())
      continue;
    ...
    else if (i == 1)
    {
      ...
      for (int j = 0; j < childrenIdx.size(); j++)
        cout << childrenIdx[j] << " ";
      cout << ")" << endl;
    }
    else if (i == 2)
