...
void HHNonXOR::generate_cauchy_matrix(int *matrix, int rows, int cols, int w)
{
    int k = cols;
    int n = rows;
    int m = n - k;
...
void HHNonXOR::Encode(ECDAG *ecdag)
{
    if (ecdag == NULL)
    {
...
void HHNonXOR::DecodeSingle(vector<int> from, vector<int> to, ECDAG *ecdag)
{
    ...
    { // special handling for the last data node
        ...
        ecdag->BindY(symbols_bindx[0], cidx_sp1[0]);
...
void HHNonXOR::SetSymbols(vector<int> symbols)
{
...
vector<int> HHNonXOR::GetRequiredSymbolsSingle(int failed_node)
{
