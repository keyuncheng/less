...
ETHHXORPlus::ETHHXORPlus(int n, int k, int w, int opt, vector<string> param)
{
    ...
    _opt = opt;
    _m = _n - _k;
    _base_w = 2; // for HHXORPlus, base code sub-packetization = 2
    ...
    if (_num_instances * _base_w != _w) {
        printf("error: invalid _w (for base code HHXORPlus, sub-packetization must be 2)\n");
        return;
    }
    ...
    // 5. init base HHXORPlus code with inverse permutated uncoupled layout
    for (int i = 0; i < _num_instances; i++) {
        vector<string> param_ins = param;
        param_ins.push_back(to_string(i)); // add the instance_id as param
        // HHXORPlus takes two additional params: 1. num_instances, instance_id
        HHXORPlus *instance = new HHXORPlus(_n, _k, _base_w, opt, param_ins);
        ...
        // 5.2 for ETHHXORPlus, there are additional symbols required for decoding
    ...
    printf("ETHHXORPlus::ETHHXORPlus finished initialization\n");
}

ETHHXORPlus::~ETHHXORPlus()
{
...
ECDAG* ETHHXORPlus::Decode(vector<int> from, vector<int> to) {
    ECDAG* ecdag = new ECDAG();
...
ECDAG *ETHHXORPlus::DecodeSingle(vector<int> from, vector<int> to) {
    ...
    if (failed_node < _k) {
        ...
        // for each instance
        for (int i = 0; i < _num_instances; i++) {
            HHXORPlus* instance = _instances[i];
...
ECDAG *ETHHXORPlus::DecodeMultiple(vector<int> from, vector<int> to) {

    ECDAG* ecdag = new ECDAG();

    // NOT implemented
    printf("ETHHXORPlus::DecodeMultiple Not implemented\n");

    return ecdag;
}

void ETHHXORPlus::Place(vector<vector<int>>& group) {
    return;
}
