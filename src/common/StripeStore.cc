...
{
  if (existEntry(filename))
    return _ssEntryMap[filename];
  else
    return NULL;
}
...
{
  if (_objEntryMap.find(objname) != _objEntryMap.end())
    return _objEntryMap[objname];
  else
    return NULL;
}
...
OfflineECPool *StripeStore::getECPool(string ecpoolid, ECPolicy *ecpolicy, int basesize)
{
  OfflineECPool *toret;
  _lockECPoolMap.lock();
  if (_offlineECPoolMap.find(ecpoolid) != _offlineECPoolMap.end())
    toret = _offlineECPoolMap[ecpoolid];
  else
  {
...
int StripeStore::getControlLoad(unsigned int ip)
{
  ...
  if (it == _controlLoadMap.end())
    toret = 0;
  else
    toret = _controlLoadMap[ip];
  _lockCLMap.unlock();
  return toret;
}
...
void StripeStore::increaseControlLoadMap(unsigned int ip, int load)
{
  ...
  unordered_map<unsigned int, int>::iterator it = _controlLoadMap.find(ip);
  if (it == _controlLoadMap.end())
    _controlLoadMap.insert(make_pair(ip, load));
  else
    _controlLoadMap[ip] += load;
  _lockCLMap.unlock();
}
...
int StripeStore::getDataLoad(unsigned int ip)
{
  ...
  if (it == _dataLoadMap.end())
    toret = 0;
  else
    toret = _dataLoadMap[ip];
  _lockDLMap.unlock();
  return toret;
}
...
void StripeStore::increaseDataLoadMap(unsigned int ip, int load)
{
  ...
  unordered_map<unsigned int, int>::iterator it = _dataLoadMap.find(ip);
  if (it == _dataLoadMap.end())
    _dataLoadMap.insert(make_pair(ip, load));
  else
    _dataLoadMap[ip] += load;
  _lockDLMap.unlock();
}
...
int StripeStore::getRepairLoad(unsigned int ip)
{
  ...
  if (it == _repairLoadMap.end())
    toret = 0;
  else
    toret = _repairLoadMap[ip];
  _lockRLMap.unlock();
  return toret;
}
...
void StripeStore::increaseRepairLoadMap(unsigned int ip, int load)
{
  ...
  unordered_map<unsigned int, int>::iterator it = _repairLoadMap.find(ip);
  if (it == _repairLoadMap.end())
    _repairLoadMap.insert(make_pair(ip, load));
  else
    _repairLoadMap[ip] += load;
  _lockRLMap.unlock();
}
...
int StripeStore::getEncodeLoad(unsigned int ip)
{
  ...
  if (it == _encodeLoadMap.end())
    toret = 0;
  else
    toret = _encodeLoadMap[ip];
  _lockELMap.unlock();
  return toret;
}
...
void StripeStore::increaseEncodeLoadMap(unsigned int ip, int load)
{
  ...
  unordered_map<unsigned int, int>::iterator it = _encodeLoadMap.find(ip);
  if (it == _encodeLoadMap.end())
    _encodeLoadMap.insert(make_pair(ip, load));
  else
    _encodeLoadMap[ip] += load;
  _lockELMap.unlock();
}
...
void StripeStore::finishECStripe(OfflineECPool *pool, string stripename)
{
  ...
  if (pos != _ECInProgress.end())
    _ECInProgress.erase(pos);
  if (_ECInProgress.size() == 0)
  {
...
vector<string> StripeStore::getLostObjs()
{
  // check whether objname is in _RPInProgress
  _lockLostMap.lock();

  // get all currently lost objs from lost map
  vector<string> toret;
  for (auto item : _lostMap)
  {
    toret.push_back(item.first);
    // mark it as repair in progress
    startRepair(item.first);
  }

  // remove them from lostMap
  _lostMap.clear();
  ...
  while (true)
  {
    ...
    while (_lostMap.size() && (rpInProgressNum < concurrentNum))
    {
      ...
      if (_conf->_repair_scheduling == "threshold")
      {
        if (!_enableRepair)
          continue;
        if (maxreq < _conf->_repair_threshold)
          continue;
      }
...
void StripeStore::finishRepair(string objname)
{
  ...
  if (pos != _RPInProgress.end())
    _RPInProgress.erase(pos);
  _lockRPInProgress.unlock();
}
