      ...
      threadPool.enqueueTask([this, agCmd]()
                             {
                               ...
                               case 5:
                                 ...
                                 break;
                               case 7:
                                 readFetchCompute(agCmd);
                                 break;
                               case 12:
                                 readDiskForShortening(agCmd);
                                 break;
                               case 13:
                                 repairObjOnline(agCmd);
                                 break;
                               default:
...
void OECWorker::clientWrite(AGCommand *agcmd)
{
  ...
  if (mode == "online")
    onlineWrite(filename, ecid, filesizeMB);
...
void OECWorker::onlineWrite(string filename, string ecid, int filesizeMB)
{
  ...
  for (int i = 0; i < eck; i++)
  {
    ...
    bool curzero = false;
    if (lastNum > 0 && i < lastNum)
      curnum = curnum + 1;
    if (lastNum > 0 && i >= lastNum)
      curzero = true;
    loadThreads[i] = thread([=]
                            { loadWorker(loadQueue[i], filename, i, eck, curnum, curzero); });
  }
  ...
  for (int i = 0; i < ecn; i++)
  {
    persistThreads[i] = thread([=]
                               { objstreams[i]->writeObj(); });
  }

  // 4. create thread to do calculation in iterations
  int stripenum = totalNumRounds;
  if (lastNum > 0)
    stripenum = totalNumRounds + 1;
  thread computeThread([=]
                       { computeWorker(computeTasks, loadQueue, objstreams, stripenum, ecn, eck, ecw); });
...
void OECWorker::offlineWrite(string filename, string ecpoolid, int filesizeMB)
{
  ...
  for (int i = 0; i < objnum; i++)
  {
    int curnum = pktnums[i];
    loadThreads[i] = thread([=]
                            { loadWorker(loadQueue[i], filename, startid, 1, curnum, false); });
    startid += curnum;
  }
                                             ...
                                             int ecw)
{
  ...
  cout << fixed << setprecision(0) << "computeWorkerDegradedOffline read + compute, " << ", start: " << start.tv_sec * 1000.0 + start.tv_usec / 1000.0 << ", end: " << end.tv_sec * 1000.0 + end.tv_usec / 1000.0 << ", duration: " << RedisUtil::duration(start, end) << endl;
    ...
    for (int taskid = 0; taskid < computeTasks.size(); taskid++)
    {
      ...
      if (stripeid == 0)
      {
        ...
        for (auto item : coefMap)
        {
          ...
          cout << "    " << target << ": " << "( ";
          for (int coefidx = 0; coefidx < coefs.size(); coefidx++)
            cout << coefs[coefidx] << " ";
          cout << ")" << endl;
        }
      }
...
void OECWorker::readDisk(AGCommand *agcmd)
{
  ...
  if (w == 1 || w == cidlist.size())
  {
    ...
    thread readThread = thread([=]
                               { objstream->readObj(slicesize); });
    BlockingQueue<OECDataPacket *> *readQueue = objstream->getQueue();
    // cacheThread
    thread cacheThread = thread([=]
                                { selectCacheWorker(readQueue, num, stripename, w, cidlist, refs); });
  ...
  else
  {
    // random read
    thread readThread = thread([=]
                               { objstream->readObj(w, cidlist, slicesize); });
    BlockingQueue<OECDataPacket *> *readQueue = objstream->getQueue();
    // cacheThrad
    thread cacheThread = thread([=]
                                { partialCacheWorker(readQueue, num, stripename, w, cidlist, refs); });
...
void OECWorker::repairObjOnline(AGCommand *agcmd)
{

  // get object name, size and pktnum
  string lostObjName = agcmd->getReadObjName();
  int lostObjSizeMB = agcmd->getFilesizeMB();
  ...
  // wait for instructions from coordinator
  string instkey = "offlinedegradedinst:" + lostObjName;
  ...
  // 0.1 ecn
  int ecn;
  memcpy((char *)&ecn, inststr, 4);
  inststr += 4;
  ecn = ntohl(ecn);
  // 0.2 eck
  int eck;
  memcpy((char *)&eck, inststr, 4);
  inststr += 4;
  eck = ntohl(eck);
  // 0.3 ecw
  int ecw;
  memcpy((char *)&ecw, inststr, 4);
  inststr += 4;
  ecw = ntohl(ecw);
  ...
  for (int loadi = 0; loadi < loadn; loadi++)
  {
    // sid
    int curidx;
    memcpy((char *)&curidx, inststr, 4);
    inststr += 4;
    curidx = ntohl(curidx);
    loadidx.push_back(curidx);
    // objname
    int len;
    memcpy((char *)&len, inststr, 4);
    inststr += 4;
    len = ntohl(len);
    char *objstr = (char *)calloc(len + 1, sizeof(char));
    memcpy(objstr, inststr, len);
    inststr += len;
    objstr[len] = '\0';
    ...
    // curlist
    int listsize;
    memcpy((char *)&listsize, inststr, 4);
    inststr += 4;
  ...
  {
    string wkey = "compute:" + lostObjName + ":" + to_string(computei);
    rReply = (redisReply *)redisCommand(waitCtx, "blpop %s 0", wkey.c_str());
    char *reqStr = rReply->element[1]->str;
    ECTask *compute = new ECTask(reqStr);
    compute->dump();
  ...
  for (int loadi = 0; loadi < loadn; loadi++)
  {
    ...
    readThreads[loadi] = thread([=]
                                { readStreams[loadi]->readObjOptimized(ecw, curlist, _conf->_pktSize / ecw); });
  }
  ...
  thread computeThread = thread([=]
                                { computeWorkerDegradedOffline(readStreams, loadidx, sid2Cids, writeQueue, lostidx, computeTasks, pktnum, ecn, eck, ecw); });
  ...
  FSObjOutputStream *objstream = new FSObjOutputStream(_conf, lostObjName, _underfs, pktnum);
...
void OECWorker::fetchCompute(AGCommand *agcmd)
{
  ...
  for (int i = 0; i < nprevs; i++)
  {
    ...
    fetchThreads[i] = thread([=]
                             { fetchWorker(fetchQueue[i], keybase, prevlocs[i], num); });
  }

  // create compute thread
  thread computeThread = thread([=]
                                { computeWorker(fetchQueue, nprevs, num, coefs, computefor, writeQueue, _conf->_pktSize / w); });
  ...
  for (int i = 0; i < computefor.size(); i++)
  {
    ...
    cacheThreads[i] = thread([=]
                             { cacheWorker(writeQueue[i], keybase, num, r); });
  }
...
void OECWorker::clientRead(AGCommand *agcmd)
{
  ...
  // 1.1 redundancy type
  int redundancy;
  memcpy((char *)&redundancy, metastr, 4);
  metastr += 4;
  redundancy = ntohl(redundancy);
  // 1.2 filesizeMB
  int filesizeMB;
  memcpy((char *)&filesizeMB, metastr, 4);
  metastr += 4;
  ...
  if (redundancy == 0)
  {
    ...
    // 0.1 ecn
    int ecn;
    memcpy((char *)&ecn, metastr, 4);
    metastr += 4;
    ecn = ntohl(ecn);
    // 0.2 eck
    int eck;
    memcpy((char *)&eck, metastr, 4);
    metastr += 4;
    eck = ntohl(eck);
    // 0.3 ecw
    int ecw;
    memcpy((char *)&ecw, metastr, 4);
    metastr += 4;
    ecw = ntohl(ecw);
  ...
  else
  {
    ...
    for (int i = 0; i < objnum; i++)
    {
      // objname
      int len;
      memcpy((char *)&len, metastr, 4);
      metastr += 4;
      len = ntohl(len);
      char *objstr = (char *)calloc(len + 1, sizeof(char));
      memcpy(objstr, metastr, len);
      metastr += len;
      objstr[len] = '\0';
...
void OECWorker::readOfflineObj(string filename, string objname, int objsizeMB, FSObjInputStream *objstream, int pktnum, int idx)
{
  ...
  if (objexist)
  {
    ...
    // 2. cache thread
    thread cacheThread = thread([=]
                                { cacheWorker(writeQueue, filename, pktnum * idx, pktnum, 1); });
    // join
    readThread.join();
    cacheThread.join();
  }
  else
  {
    ...
    else
    {
      ...
      // stripename
      int stripenamelen;
      memcpy((char *)&stripenamelen, inststr, 4);
      inststr += 4;
      stripenamelen = ntohl(stripenamelen);
      char *stripenamestr = (char *)calloc(stripenamelen, sizeof(char));
      memcpy(stripenamestr, inststr, stripenamelen);
      inststr += stripenamelen;
      string stripename(stripenamestr);
      free(stripenamestr);
      // num
      int num;
      memcpy((char *)&num, inststr, 4);
      inststr += 4;
      num = ntohl(num);
      vector<int> cidxlist;
      ...
      {
        int cidx;
        memcpy((char *)&cidx, inststr, 4);
        inststr += 4;
        cidx = ntohl(cidx);
        cidxlist.push_back(cidx);
        unsigned int ip;
        memcpy((char *)&ip, inststr, 4);
        inststr += 4;
        ip = ntohl(ip);
        iplist.push_back(ip);
      ...
      for (int i = 0; i < num; i++)
      {
        ...
        fetchThreads[i] = thread([=]
                                 { fetchWorker(fetchQueue[i], keybase, iplist[i], pktnum); });
      }
...
void OECWorker::readFetchCompute(AGCommand *agCmd)
{
  ...
  for (int i = 0; i < nprevs; i++)
  {
    ...
    {
      fetchThreads[i] = thread([=]
                               { objstream->readObj(pktsize); });
    }
    else
    {
      string keybase = stripename + ":" + to_string(prevCids[i]);
      fetchThreads[i] = thread([=]
                               { fetchWorker(fetchQueue[i], keybase, prevLocs[i], pktnum); });
    }
  }

  // create compute thread
  thread computeThread = thread([=]
                                { computeWorker(fetchQueue, nprevs, prevCids, pktnum, coefs, computefor, writeQueue, slicesize); });
  ...
  for (auto item : cacheRefs)
  {
    ...
    cacheThreads[cacheid++] = thread([=]
                                     { cacheWorker(queue, keybase, pktnum, ref); });
  }
