...
while IFS= read -r line || [[ -n "$line" ]]; do
    ...
    python3 extract_testbed_logs_full_node.py \
        -r "$numRuns" \
        -d "$proj_dir/eval_results/full-node/${code_id}/bw${bandwidth_Gbps}Gbps_blk${block_size_MiB}MiB_pkt${packet_size_KiB}KiB"
